...
public class BoggleBoard2 {
	...
	public static void dfs(int i,int j,char[][] board,Trie root,List<String>list)
	{

		if(root.val)
		{
			System.out.println("word "+root.value +" at ("+i+","+j+")");
			list.add(root.value);
		...
		board[i][j]=' ';
		dfs(i-1,j,board,next,list) ;
		dfs(i+1,j,board,next,list) ;
		dfs(i,j-1,board,next,list) ;
		dfs(i,j+1,board,next,list);

		dfs(i-1,j-1,board,next,list) ;
		dfs(i+1,j+1,board,next,list);
		dfs(i-1,j+1,board,next,list) ;
		dfs(i+1,j-1,board,next,list);
